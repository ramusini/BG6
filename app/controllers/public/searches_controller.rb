class Public::SearchesController < ApplicationController
  ...
  def search
    ...
    if params[:bg_title].present?
      ...
      @result = res["item"].map{|item| {id: item["id"], image: item["image"], title: item.dig("name", 0, "value"), minplayer: item.dig("minplayers", 0, "value"), maxplayer: item.dig("maxplayers", 0, "value"), playingtime: item.dig("playingtime", 0, "value")}}
    end
  end
